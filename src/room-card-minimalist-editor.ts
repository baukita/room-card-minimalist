...
export class RoomCardEditor extends LitElement {
	...
	/**
	 * Add a new stat (template type by default)
	 */
	private _addStat(): void {
		if (!this._config) return;

		const stats = [...(this._config.stats || [])];
		stats.push({ type: 'template', icon: 'mdi:chart-bar', value: '' });

		this._dispatchConfigChanged({ ...this._config, stats });
	}

	/**
	 * Delete a stat at the given index
	 */
	private _deleteStat(idx: number): void {
		if (!this._config) return;
	...
	/**
	 * Move a stat up or down
	 */
	private _moveStat(idx: number, pos: number): void {
		if (!this._config) return;
	...
	/**
	 * Handle stat form value changes
	 */
	private _valueChangedStat(statIndex: number, ev: CustomEvent): void {
		if (!this._config || !this.hass) return;

		const stats = [...(this._config.stats || [])];
		stats[statIndex] = ev.detail.value;

		this._dispatchConfigChanged({ ...this._config, stats });
	}

	/**
	 * Get the icon for a stat configuration
	 */
	private _getStatIcon(stat: StatConfig): string {
		if (stat.icon) {
			return stat.icon;
		}
	...
	/**
	 * Get display name for a stat configuration
	 */
	private _getStatDisplayName(stat: StatConfig, index: number): string {
		if (stat.label) {
			return stat.label;
		}
		if (stat.type === 'entity' && stat.entity && this.hass) {
	...
	/**
	 * Get form schema for a stat configuration
	 */
	private _getStatSchema(stat: StatConfig): SchemaItem[] {
					...
					<div class="box">
						<div class="entity-header">
							<div class="entity-info">
								<div
									class="drag-handle"
									@mousedown=${(ev: MouseEvent) => this._handleMouseDown(ev, stat_idx)}
								>
									<ha-icon .icon=${'mdi:drag'}></ha-icon>
								</div>
								<ha-icon .icon=${this._getStatIcon(stat)} class="entity-icon"></ha-icon>
							...
							<div class="entity-controls">
								...
								<mwc-icon-button
									.disabled=${stat_idx === (this._config?.stats?.length || 0) - 1}
									@click=${() => this._moveStat(stat_idx, 1)}
						...
						<ha-form
							...
							@value-changed=${(ev: CustomEvent) => this._valueChangedStat(stat_idx, ev)}
						></ha-form>
					</div>
				`
			)}
