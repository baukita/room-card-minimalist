...
export class RoomCard extends LitElement {
	...
	private _subscribeTemplates(): void {
		...
		// Subscribe to stat value templates
		for (const stat of this._config.stats) {
			if (stat.type === 'template' && stat.value) {
				this._templateService.subscribe(stat.value);
			}
		}
	}
	...
	private _renderStats(): TemplateResult | string {
		if (!this._config?.stats || this._config.stats.length === 0) {
			return '';
		}
